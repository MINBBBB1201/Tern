...
## I13 — 모바일 히어로 잔여 2건: 궤도 하단부 + 티켓/지구본 겹침 (2026-08-08)

I12 배포 후 실기기(iPhone Safari) 확인에서 남은 2건. 신고가 "살짝 이상함"으로
모호했으므로, **무엇이 문제였는지 확정하는 것**이 이 라운드의 1차 산출물이다.

### I13-0. 진단 — 둘 다 실재. 서로 다른 결함이고, I12 회귀는 아니다

측정 방식: headful Chrome(헤드리스 SwiftShader 는 2–4fps 라 대상 자체를 왜곡,
I6-b), Accept-Language 고정(I9), **지구본 자전 위상 고정**. 마지막 항목이 이번에
새로 필요했다 — 궤도는 자전하는 프레임 안의 원이므로 (`spin.localToWorld`),
**화면상 궤적은 페이지를 언제 열었느냐에 따라 달라진다**(자전 주기 ~70초).
고정하지 않으면 before/after 가 애초에 같은 비행이 아니다.

**(1) "궤도가 아래쪽일 때 이상함" = 렌더링 결함이 아니라 레이아웃 충돌.**
끊김도, 스테이지 클리핑도, 궤도 형태 왜곡도 아니었다. 궤도 불변량
`dist/radius` 는 한 바퀴 내내 4개 폭 전부에서 **1.160** 으로 유지된다.

실제 원인: 세로 화면에서 **궤도 하단부가 히어로 카피 위를 지나간다.**
390×844 에서 한 바퀴를 16프레임으로 측정한 결과 —

| | 헤드라인 박스 덮는 비율 | 본문 문단 덮는 비율 |
|---|---|---|
| 데스크톱 1440 | 0% | 0% |
| 폰 390 | 최대 **38.6%** | 최대 **28.8%** |

데스크톱은 카피가 오른쪽 절반이고 지구본·궤도가 왼쪽이라 원리적으로 겹치지
않는다. 세로는 카피가 **중앙 전폭**이고 지구본 바로 아래라 궤도 하단부가 반드시
지나간다. 게다가 그 구간에서 새는 카메라에 대해 거의 **엣지온**이라
`143 × 42px` 의 창백한 유리 조각이 16px 본문 위에 **가로로 눕는다** — 새로
읽히지 않고 긁힘/아티팩트로 읽힌다. 그것이 "이상함"의 정체다
(`i13-390-orbit-cycle-11-before.png`).

궤도를 옮겨서는 해결할 수 없다: 반지름이 지구본 반지름의 1.16배로 고정(이번
라운드가 보존해야 할 불변량)이고, **지구본 원반 자체가 이미 카피 전체를 덮고
있다.**

**(2) 티켓 카드 겹침 = 착시가 아니라 실제 픽셀 겹침.**
지구본 실루엣을 타원으로 두고 정규화 반지름 k 를 재면(k=1 이 윤곽선 위) 카드
좌하단 모서리가 **k = 0.73**, 즉 윤곽선 안쪽 **32–36px** 이다. 게다가 카드는
z=0, 그 지점의 구면은 z≈−0.07 이라 카드가 **구면 앞**에 있다 — 지구본과 항로선
위에 그려진다. 3개 폭 전부 동일:

| 폭 | 카드↔윤곽선 최소거리 (before) |
|---|---|
| 360 | **−32.1px** |
| 390 | **−33.5px** |
| 414 | **−35.6px** |
...
**근본 원인 (I12 잔여분이 아니라, I12 가 고친 구조가 드러낸 별개 결함).**
`PortraitComposition` 은 씬 전체를 **뷰 중심 기준으로 축소**한다. 그래서
지구본이 원래 앵커인 화면폭 0.26 에서 **0.38 로 끌려 들어오고**, 우측 윤곽선이
데스크톱의 화면폭 45% 에서 세로에서는 **71%** 로 밀린다. 탑승권이 차지하도록
구성된 우측 빈 컬럼이 세로에서는 **존재하지 않는다.** I12 코드 주석의 전제
("지구본 우측 윤곽은 스테이지 폭 ~0.58 에서 끝나고 카드는 0.61 에서 시작")는
측정과 맞지 않았다 — 실제로는 0.71 에서 끝나고 카드는 0.53 에서 시작했다.

따라서 **위치만으로는 못 고친다**: 카드를 원래 크기로 두면 화면 안 + 내비 아래를
만족하는 최적 위치에서도 윤곽선에 14px 못 미친다(계산·측정 일치).

### I13-1. 수정

- **`COPY_DIM` (HeroTernView)** — 세로에서만, 새와 트레일이 **카피 박스 위를 지날
  때** 0.30 으로 감광. 상수가 아니라 **밴드**다: 하늘과 지구본 상단부에서는
  100% 유지되므로 시퀀스는 그대로 읽힌다. 밴드는 `.hero-copy` 의 실제 DOM 박스에서
  측정한다(폰트 로딩·번역 길이·회전에 따라 움직이므로 하드코딩하지 않음). 트레일은
  **샘플별**로 감광 — 새 위치로 트레일 전체를 감광하면 아직 지구본 위를 지나는
  절반까지 어두워진다. 선례: HeroGlobe 가 이미 같은 이유로 세로에서 0.42 로
  감광하고 있었다. 배경에 적용된 규칙을 **배우에게도** 적용한 것.
- **`SETTLE_PORTRAIT` + `PASS_PORTRAIT_SCALE`** — 세로 탑승권을 0.76 배로 줄이고
  우상단 코너로 이동(`{x:0.822, y:0.122}`). 랜드스케이프는 램프가 0 이라 무변경.
  트레일이 응결하는 `outlinePoint` 타깃도 같은 배율을 받는다 — 안 그러면 카드가
  더 이상 그 크기가 아닌 사각형으로 응결해 모프가 깨진다.

**결과 (`i13-verify-{before,after}.json`)**

| 폭 | 카드↔윤곽선 before → after | 카드 폭 | 우측여백 | 내비 아래 | 화면 안 |
|---|---|---|---|---|---|
| 360 | −32.1 → **+18.4px** | 135→103 | 12.1 | 4.3 | ✓ |
| 390 | −33.5 → **+19.7px** | 147→113 | 13.4 | 9.1 | ✓ |
| 414 | −35.6 → **+20.9px** | 156→120 | 14.2 | 13.7 | ✓ |
| 데스크톱 | 250.1 → **250.1px** | 355→355 | 221.7 | 45.6 | ✓ |

새가 카피 위에 있을 때의 불투명도: **1.0 → 0.30–0.32**. 궤도 불변량 1.160 유지.
모프 무손상 확인(`i13-390-morph-t{4600,5000,5400,5800}.png`).

### 범위 밖 발견 (이번 라운드에서 건드리지 않음)

- **엔트리 스우프가 지구본을 통과한다.** 800ms 엔트리 베지어에서 `dist/radius` 가
  자전 위상에 따라 **1.0 아래로** 내려간다(390×844 에서 0.735 관측; 고정 위상에서는
  1.045). 1 미만이면 새가 구 내부에 있어 깊이 테스트로 가려졌다가 다시 튀어나온다.
  신고된 증상이 아니고 궤도 구간도 아니라 보류.
- **프레임 거버너 강등이 비가역이다.** `onFallback → floorQuality()` 는 세션 내에서
  되돌릴 수 없다. 검증 스크립트가 스크린샷 스톨만으로 히어로를 static 으로 떨어뜨렸다
  (그래서 스크립트가 티어를 붙잡는다). 실사용에서 일시적 히칭이 그 페이지뷰 내내
  히어로를 영구 강등시킬 수 있다 — 실트래픽 영향은 미확인.
- **`.hero-content { padding-top: 184px }` 가 이제 과잉이다.** I12 가 예전(더 큰)
  탑승권 하단(76–157px)에 맞춰 정한 값인데, 새 탑승권 하단은 123–137px 이다.
  헤드라인을 ~20px 올릴 여지가 있으나 레이아웃 변경이라 범위 밖.
